"""
    This file creates input data for autogluon.
"""
...
def create_input_multiprocess(ids):
    ...
    autogluondfs = pd.concat(pool_results, ignore_index=True) 

    # release memory
    pool_results = None
    gc.collect()

    autogluondata = to_autogluondata(autogluondfs)

    # release memory
    autogluondfs = None
    gc.collect()

    return autogluondata

def create_input(ids):
    df = ids.copy()
    df = create_dataframes_with_features(df)
    autogluondfs = to_autogluon_df(df)
...
def to_autogluon_df(_df):
    """Convert dataframe into autogluon values dataframe."""

    LOG.info("start to_autogluon_df")
...
def to_autogluondata(_df):
    """Convert autogluon values dataframe into dictionary of labels, weights and data."""
    labels = _df.values[:,0]
