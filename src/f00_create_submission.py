...
predict_autogluon = pd.read_csv("../model/autogluon_predicted.txt", header=None, names=["prob"])
...
predict_df["predict_score"] = np.log( predict_autogluon["prob"].values / (1 - predict_autogluon["prob"].values) )
...
def create_answer(_predict_df):
    ...
    answer_df.to_csv("../model/predict_coupons_autogluon.csv", index=False)
